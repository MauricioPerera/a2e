...
class CredentialsVault:
    ...
    def __init__(
        ...
        if master_key:
            ...
            env_key = os.environ.get("A2E_VAULT_MASTER_KEY")
            if env_key:
                self.master_key = env_key.encode() if isinstance(env_key, str) else env_key
                logger.info("Vault master key loaded from A2E_VAULT_MASTER_KEY environment variable")
            else:
                logger.warning(
                    "No master key provided and A2E_VAULT_MASTER_KEY not set. "
                    "Generating random key — vault data will NOT persist across restarts."
                )
        ...
        if use_rag:
            if A2ERAGSystem is None:
                ...
                try:
                    ...
                    logger.warning(f"RAG system could not be initialized: {e}. Credential search will use keywords only.")
                    self.use_rag = False
                    self.rag = None
        else:
            self.rag = None
    ...
    def _derive_key(self, master_key: bytes) -> bytes:
        ...
        """
        salt = os.environ.get("A2E_VAULT_SALT", "a2ui_vault_salt").encode()
        kdf = PBKDF2HMAC(
